// Self-contained Navbar Module - Updated with Dynamic User Info
const CVisionNavbar = (function() {
    ...
    async function loadUserInfo() {
        ...
        if (!token) {
            console.warn('Navbar: No authentication token found');
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 401) {
                    console.warn('Navbar: Token expired or invalid');
                    // Clear invalid token
            ...
            localStorage.setItem('cvision_user', JSON.stringify(userInfo));
            
            console.log('Navbar: User info loaded successfully', userInfo);
    ...
    async function markAsRead(notificationId) {
        ...
        try {
            ...
            console.error('Navbar: Error marking as read:', error);
        }
    }
